...
import { getCategories, getTools } from "@/lib/queries";
import { serializeCategory, serializeTool, type SerializedTool } from "@/lib/serializers";

export const metadata = {
  title: "AI Tool Categories — AI BirdView",
...
export default async function CategoriesPage() {
  const [categoriesRaw, toolsRaw] = await Promise.all([getCategories(), getTools()]);
  const categories = categoriesRaw.map(serializeCategory);
  const tools = toolsRaw.map(serializeTool);
  const samplesByCat: Record<string, SerializedTool[]> = {};
  for (const t of tools) {
    if (!samplesByCat[t.category.slug]) samplesByCat[t.category.slug] = [];
    if (samplesByCat[t.category.slug].length < 3) samplesByCat[t.category.slug].push(t);
  }
    ...
    <div>
      ...
      <section className="mx-auto max-w-7xl px-5 lg:px-8 py-16 md:py-24">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((c) => {
            const sampleTools = samplesByCat[c.slug] ?? [];
            return (
              <Link
                key={c.slug}
