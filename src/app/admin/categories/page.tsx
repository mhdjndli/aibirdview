...
export default async function AdminCategoriesPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {sp.saved && (
        <p className="rounded-xl border border-kiwi-200 bg-kiwi-50/60 px-4 py-2.5 text-[13px] text-kiwi-800">Saved.</p>
      )}
      {sp.deleted && (
        <p className="rounded-xl border border-ink-200 bg-ink-50 px-4 py-2.5 text-[13px] text-ink-700">Deleted.</p>
      )}
            ...
            {cats.map((c, i) => (
              <tr key={c.id} className="border-t border-ink-100">
                <td className="px-4 py-3">
                  <CategoryRowActions id={c.id} canUp={i > 0} canDown={i < cats.length - 1} />
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-xl bg-ink-100 text-[18px] ${c.accent}`}>
                      {c.glyph}
                    </span>
                    <div>
                      <div className="font-medium text-ink-900">{c.name}</div>
                      <div className="text-[12px] text-ink-500">/{c.slug}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-ink-600 max-w-md">{c.tagline}</td>
                <td className="px-4 py-3 text-ink-700">{c._count.tools}</td>
